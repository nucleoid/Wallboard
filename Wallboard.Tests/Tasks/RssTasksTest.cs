...
namespace Wallboard.Tests.Tasks
{
    ...
    public class RssTasksTest
    {
        ...
        [Test]
        public void LoadItems_Reader_Loads_Items_From_Reader()
        {
            //Arrange
            var reader = XmlReader.Create(new StringReader(TestRssFeed));

            //Act
            var items = _tasks.LoadItems(reader);

            Assert.AreEqual(2, items.Count());
            Assert.AreEqual("CLUSCPP-DEV-2 has FAILED : Updated by James Ehly", items.FirstOrDefault().Title.Text);
        }
        ...
        private string TestRssFeed
        {
            get
            {
                return "<?xml version=\"1.0\" encoding=\"UTF-8\"?><rss xmlns:dc=\"http://purl.org/dc/elements/1.1/\" version=\"2.0\"><channel>" +
                       ...
                       "This feed is updated whenever a build gets built</description><item><title>CLUSCPP-DEV-2 has FAILED : Updated by James Ehly</title>" +
                       ...
                       "<guid>http://tools.sycorr.com/build/browse/CLUSCPP-DEV-2</guid><dc:date>2011-11-10T05:40:23Z</dc:date></item><item><title>" +
                       ...
                       "<dc:date>2011-11-04T03:28:28Z</dc:date></item></channel></rss>";
            }
        }
    }
}
