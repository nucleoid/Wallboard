    ...
    $.fn.extend({
        dnews: function (options) {
            ...
            return this.each(function () {
                ...
                if (settings.feedurl != '') {
                    $('.news', wrapper).html('<img src="/Content/images/dnews/loading.gif" alt="loading" style="margin-top:6px;margin-left:6px;"/>');
                    ...
                    function feedLoaded(result) {
                        if (!result.error) {
                            ...
                            allNews.each(function (index, news) {
                                ...
                                if (!settings.twitterimages && headlintext.length > 100)
                                    headlintext = headlintext.substr(0, 100) + ' ...';
                                var twitterProfileImage = '';
                                if (settings.twitterimages) {
                                    var twitterProfile = $(news).find('link').text().replace(/http:\/\/twitter.com\//, '').replace(/\/statuses\/.*/, '');
                                    twitterProfileImage = "<img class='twitter-headline' src='https://api.twitter.com/1/users/profile_image?screen_name=" + twitterProfile + "&size=mini' />";
                                }
                                var headline = $('<div/>').addClass('headline').append(twitterProfileImage).append($('<a/>').attr('id', id).html(headlintext).attr('href', $(news).find('link').text()));
                ...
                function setupHeadlines() {
                    ...
                    doublecontainerWidth = ($('.news', wrapper).width() + $('.news-title', wrapper).width()) * 2;
                    allHeadlines = $('.headline', wrapper);
                    left = wrapper.find('.news').css('left');
                    if (left == 'auto') left = $('.news-title').width() + 'px';
                    allHeadlines.css('left', '-' + doublecontainerWidth + 'px');
...
//                    });
                ...
                function switchNews() {
                    if (direction == 'forward') {
                        ...
                        $(allHeadlines[prevIndex]).stop().animate({ 'left': doublecontainerWidth + 'px' }, "slow", function () {
                            $(this).css('left', '-' + doublecontainerWidth + 'px');
                        });
                        $(allHeadlines[curIndex]).stop().animate({ 'left': left }, "slow");
                    }
                    else {
                        curIndex = (prevIndex > 0 ? prevIndex - 1 : total - 1) % total;
                        $(allHeadlines[prevIndex]).stop().animate({ 'left': -doublecontainerWidth + 'px' }, "slow");
                        $(allHeadlines[curIndex]).css({ 'left': doublecontainerWidth + 'px' }).stop().animate({ 'left': left }, "slow");
                    }
                    prevIndex = curIndex;
                }
